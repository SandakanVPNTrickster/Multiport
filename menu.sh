...
Exp=$(curl -sS https://raw.githubusercontent.com/SandakanVPNTrickster/Multiport/main/main/allow | grep $MYIP | awk '{print $3}')
fi
clear
echo -e "\e[36m╒════════════════════════════════════════════╕\033[0m"
...
if [[ $(cat /opt/.ver) = $serverV ]] > /dev/null 2>&1; then
    ...
    if [[ $x -eq 69 ]]; then
       wget -q -O /usr/bin/update-script "https://raw.githubusercontent.com/SandakanVPNTrickster/Multiport/main/dll/system/update-script.sh" && chmod +x /usr/bin/update-script
